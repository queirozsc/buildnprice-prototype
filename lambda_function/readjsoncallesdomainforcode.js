        ...
        if (err) {
           ...
           // console.log(jsonData);
            ...
            for(i=0;i<jsonData['resultJson'].length;i++)
            {
                ...
                callESDomainService(desc,quant,tempJson,i,arrLength,recordProcessCount,bucket,key);
            }
...
function callESDomainService(desc,quant,tempJson,count,arrLength,recordProcessCount,bucket,key)
{
    try{
    var d="",m="",s="",c="",b="";
    https.get('https://search-buildnprice-supplier-sample-liyot4gwziyvouk7n5kagjehg4.us-east-1.es.amazonaws.com/supplier/products/_search?q="'+desc+'"', (resp) => {
             ...
             {
                console.log(resData['hits']['hits'].length);
			...
			if( tempJson['searchCodes'].length == arrLength || recordProcessCount.length==arrLength)
