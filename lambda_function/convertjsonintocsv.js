...
exports.handler = (event, context, callback) => {
    ...
    s3.getObject(params, (err, data) => {
        if (err) {
            ...
            var emailLis=getSubscriptionEmailList(fileName,jsonData,bucket,key);
...
function getSubscriptionEmailList(fileName,jsonData,bucket,key)
{
    var emailList=[];
    try
    {
        ...
        snsClient.listSubscriptionsByTopic(params, function(err, data) {
          if (err) console.log(err, err.stack); // an error occurred
              ...
              generateCSVFile(emailList,fileName,jsonData,bucket,key);
          }
        });
    return emailList;
    }catch(ex){
...
function generateCSVFile(emailList,fileName,jsonData,bucket,key)
{
    var desc="";
    var i=0;
    var row="";
